# Try dynamic-oracle training
# To include cpu compatibility, no feat-column used
...
function initmodel1(o, s; ftype=Float32)
    ...
    optims = optimizers(model, o[:optimization], gclip=o[:gclip])
    return model, optims
...
function cache_bufbatch(model, walls, parsers, hidden; pdrop=0.0) #dropout
    yalls = Array{Any}(length(parsers)) # i for ith parser
    B = length(parsers)
    for i in 1:B
        wvecs = walls[i];
        p = parsers[i]
        youts, cyouts = scan_buffer(model, wvecs, p, hidden, pdrop=pdrop) #dropout
...
function scan_stack(model, wvecs, parser, hidden, embdim; pdrop=0.0) #dropout
    sentence = parser.sentence
    T = parser.sptr
    ...
    allin = dropout(allin, pdrop) # dropout
...
function scan_stackbatch(model, walls, parsers, hidden, embdim; pdrop=0.0)#dropout
    B = length(parsers)
    yalls = Any[]
    for i in 1:B
        p = parsers[i]; wvecs = walls[i];
        yst = scan_stack(model, wvecs, p, hidden, embdim, pdrop=pdrop)#dropout
        push!(yalls, yst)
    end
    yalls = cat1d(yalls...)
    ncols = B
...
function scan_action(model, actions, state; acdrop=0.0) #dropout
    acemb, actmodel = actembed(model), actm(model)
    yout0, hout0, cout0 = state[1], state[2], state[3]
    input = acemb[:, actions]
    input = dropout(input, acdrop) #dropout
...
function update_cache!(model, wvecs, cacbufs, p, m; pdrops=(0.0, 0.0)) # dropout
    wdrop, cachedrop = pdrops[1], pdrops[2]
    ...
    elseif m % 2 == 0 # left
        ...
        input = dropout(input, wdrop) # dropout
        ...
        i2r2 = dropout(i2r2, cachedrop) #dropout
...
function oracleloss(allmodel, sentences, arctype, hiddens, embdim;losses=nothing, pdrop=(0.0, 0.0),ldrops=(0.5, 0.5, (0.5, 0.5)))
    stackdrop, bufdrop, cachedrop = ldrops
    ...
    cachedbufs = cache_bufbatch(allmodel, walls, parsers, hidbuf, pdrop=bufdrop)
    ...
    while !all(parserdone)
        ...
        ysts   = scan_stackbatch(allmodel, walls, parsers, hidstack, embdim, pdrop=stackdrop)
        ...
        # dynamic-related
        logvals = Array(AutoGrad.getval(logprobs))
        ...
        for (i, p) in enumerate(parsers)
            ...
            if mcosts[goldmove] == typemax(Cost)
                ...
                # dynamic
                isorted = sortperm(logvals[:,i], rev=true) # ith col ith inst.
                for m in isorted
                    if moveok(p, m)
                        # update before moving to the next step
                        update_cache!(allmodel, walls[i], cachedbufs[i], p, m, pdrops=cachedrop)
                        move!(p, m); push!(inacts, m);
                        break
                    end
                end
                if losses != nothing
